...
def create_synthetic_data(samples=10000) -> pd.DataFrame:
    """Create realistic synthetic transaction data"""
    data = []
    
    for i in range(samples):
        # Generate user ID for consistency
        ...
        # Generate amount with different distributions for fraud vs normal
        if is_fraud:
            # Fraud transactions tend to be either very high or suspiciously specific amounts
            if random.random() < 0.3:
                amount = random.uniform(10000, 50000)  # High amount fraud
            else:
                amount = random.choice([999.99, 1999.99, 4999.99])  # Specific amounts
        else:
            # Normal transactions follow a more realistic distribution
            amount = np.random.lognormal(mean=4, sigma=1.5)  # Log-normal distribution
            amount = max(1, min(amount, 5000))  # Clamp between $1 and $5000
        
        # Select category with fraud bias
        if is_fraud:
            # Fraud more likely in certain categories
            category_weights = [0.1, 0.15, 0.25, 0.1, 0.2, 0.15, 0.05]  # Higher weights for risky categories
            category = random.choices(CATEGORIES[:7], weights=category_weights)[0]
        else:
        ...
        hour = random.choices(
            list(range(24)),
            weights=[1,1,1,2,3,5,8,10,12,15,18,20,22,20,18,15,12,10,8,6,4,3,2,1]
        )[0]
        ...
        # Device ID (fraud often uses new/unknown devices)
        if is_fraud and random.random() < 0.4:
            device_id = f"device_{uuid.uuid4().hex[:8]}"  # New device
        else:
            device_id = f"device_{random.randint(1, 500)}"  # Known device
        
        data.append({
            'user_id': user_id,
            'amount': amount,
            'category': category,
            'lat': loc['lat'] + random.uniform(-0.1, 0.1),  # Small location variation
...
class FraudMLEngine:
    ...
    def train_models(self):
        ...
        df = create_synthetic_data(20000)
    ...
    def evaluate_transaction(self, amount: float, category: str, lat: float, lng: float, 
                           user_id: str, device_id: str = None) -> Tuple[bool, int, str]:
    ...
    def _apply_rules(self, transaction: Dict[str, Any], user_hist: Dict[str, Any]) -> Tuple[float, List[str]]:
        ...
        if transaction['category'] in HIGH_RISK_CATEGORIES:
            if transaction['amount'] > 5000:
                risk_score += 0.5
                reasons.append(f"High amount in {transaction['category']}")
            else:
                risk_score += 0.2
        ...
        if user_hist.get('count', 0) < 3:
            if transaction['amount'] > 1000:
                risk_score += 0.3
                reasons.append("New user with high transaction")
